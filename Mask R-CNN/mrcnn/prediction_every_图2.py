...
IMAGE_DIR = os.path.join(ROOT_DIR, "test_images/wheat_every/000002")
...
path = '/home/yu/Mask_RCNN/test_images/wheat_every/000002'
...
for i in range(len(result_list)):
    index = result_list[i].index(max(result_list[i]))
    print(result_list[i], "\n", index, max(result_list[i]))
    result_dict[index] = max(result_list[i])
    # result_dict[result_list.index(max(result_list[i]))] = max(result_list[i])

    # # result_dict[i] = max(result_list[i])
